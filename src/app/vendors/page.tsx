...
import { faAngleDown, faBowlFood, faLocationArrow, faLocationPin, faPerson, faRestroom, faRupee, faSearch, faSort, faStar } from "@fortawesome/free-solid-svg-icons";
        ...
        <div className="md:px-1">
          <div className="header px-4">
            <div className="text-gray-400  text-xs md:text-sm flex gap-2 pb-3 pt-5">
              <p><span>Home {">"}</span></p>
              <p><span>Vendors {">"}</span></p>
              <p><span>Wedding Venues </span></p>
            </div>
            {!isMobile ? (
              <DesktopHeader />) : (
              <MobileHeader />
    ...
    <div className="flex  justify-between items-center pt-2 pb-4">
      ...
      <div className="flex gap-4 mb-2">
        <div className="flex items-center shadow-sm rounded-lg">
          <FontAwesomeIcon icon={faSearch} className="w-6 pl-3 py-2.5 text-gray-500 border border-gray-200 border-r-0 rounded-l-lg" />
          <input
            type="text"
            className="outline-none border p-1.5 border-gray-200 border-l-0  px-2 rounded-r-lg w-[300px]"
            placeholder="Search..."
          />
        </div>
      </div>
